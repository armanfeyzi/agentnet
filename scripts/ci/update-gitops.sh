...
API_VALUES_BASE="apps/backend/agentnet-api/base/values.yaml"
WEB_VALUES_BASE="apps/frontend/agentnet-web/base/values.yaml"
...
for file in "$API_VALUES_BASE" "$WEB_VALUES_BASE" "$API_VALUES" "$WEB_VALUES" "$API_APPSET" "$WEB_APPSET" "$PG_APPSET"; do
  if [[ ! -f "$file" ]]; then
...
yq -i ".image.tag = \"${IMAGE_TAG}\"" "$API_VALUES_BASE"
yq -i ".image.tag = \"${IMAGE_TAG}\"" "$WEB_VALUES_BASE"
...
git add "$API_VALUES_BASE" "$WEB_VALUES_BASE" "$API_VALUES" "$WEB_VALUES" "$API_APPSET" "$WEB_APPSET" "$PG_APPSET"
